using System;
using System.Collections.Generic;
using System.Text;

namespace ZyraHangfireService.Cosntaants
{
    public static class HRMConstants
    {
        public const string SHIFT_START_TIME_NAME = "SHIFT_START_TIME";

        public const string SHIFT_END_TIME_NAME = "SHIFT_END_TIME";
        public const string SHIFT_GRACE_PERIOD_NAME = "GRACE_PERIOD_MINS";

        public const string SHIFT_AUTO_CHECKOUT_NAME = "AUTO_CHECKOUT_TIME";
    }
}

...
namespace ZYRAHRM.IntegrationApp.Controllers
{
    ...
    public class HolidayController : ControllerBase
    {
        ...
        public HolidayController(AttendanceDbContext dbContext, ILogger<HolidayController> logger)
        {
        ...
        public async Task<IActionResult> Get()
        {
            try
            {
                _logger.LogInformation("Fetching Holiday mappings...");

                var holidays = await _dbContext.Holidiays.ToListAsync();

                if (holidays == null || !holidays.Any())
                {
                    var msgInfo = "No holiday found.";
                    _logger.LogWarning(msgInfo);
                    return NotFound(msgInfo);
                }

                _logger.LogInformation("Fetched {Count} holidays.", holidays.Count);
                return Ok(holidays);
            }
            catch (Exception ex)
            {
